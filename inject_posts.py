...
from blogApp.models import Post  # Replace 'yourapp' with your actual app name
...
posts_json = [
    ...
    {
        "title": "List of PyCon 9018 Talks",
...
for post in posts_json:
    if post["user_id"] in valid_user_ids:
        post_instance = Post(
            title=post["title"], content=post["content"], author_id=post["user_id"]
        )
        post_instance.save()
    else:
        print(f"Invalid user_id: {post['user_id']} for post: {post['title']}")
